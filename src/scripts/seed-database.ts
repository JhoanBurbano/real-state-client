...
import { SEED_DATA } from '../supabase/functions/server/seeds.ts';
...
class DatabaseSeeder {
  ...
  async seedProperties() {
    ...
    for (const property of SEED_DATA.properties) {
      ...
      console.log(`   ✅ ${property.address} - $${property.price.toLocaleString()} (${property.city})`);
    }
    
    console.log(`✨ Successfully seeded ${SEED_DATA.properties.length} properties\n`);
  }
  ...
  async seedSampleLeads() {
    ...
    const sampleLeads = [
      ...
      },
      {
        ...
        message: 'Need to sell Coral Gables estate quickly. Recently relocated to New York.',
    ...
    for (const lead of sampleLeads) {
      ...
      console.log(`   ✅ ${lead.firstName} ${lead.lastName} - ${lead.interest} (${lead.status})`);
    }
  ...
  async seedNewsletterSubscriptions() {
    ...
    console.log(`✨ Successfully seeded ${sampleSubscriptions.length} newsletter subscriptions\n`);
  }
  ...
  async printSummary() {
    ...
    const prices = properties.map(item => item.value.price).sort((a, b) => a - b);
    if (prices.length > 0) {
      console.log('\nPrice Range Analysis:');
  ...
  async validateData() {
    ...
    for (const item of properties) {
      ...
      if (property.price < 100000 || property.price > 100000000) {
        console.error(`❌ Property ${property.id} has unrealistic price: $${property.price}`);
        validationErrors++;
      }
      ...
      if (property.agentId) {
        ...
        if (!agent) {
          console.error(`❌ Property ${property.id} references non-existent agent: ${property.agentId}`);
          validationErrors++;
        }
      }
    }
    ...
    for (const item of agents) {
      ...
      if (!agent.licenseNumber || !agent.licenseNumber.startsWith('FL-')) {
        console.error(`❌ Agent ${agent.id} has invalid Florida license: ${agent.licenseNumber}`);
        validationErrors++;
      }
    }
  ...
  try {
    switch (command) {
      ...
      case 'summary':
        ...
        break;
      default:
        console.log('Usage: deno run seed-database.ts [seed|reset|clear|validate|summary]');
        break;
    }
  } catch (error) {
