...
async function initializeLuxuryProperties() {
  try {
    ...
    if (existingProperties.length === 0) {
      console.log(
        '🏖️ Initializing MILLION Real Estate luxury property portfolio...'
      )
      ...
      for (const agent of SEED_DATA.agents) {
        ...
        console.log(
          `   ✅ Agent: ${agent.firstName} ${agent.lastName} - ${agent.title}`
        )
      }
      ...
      for (const property of SEED_DATA.properties) {
        ...
        if (isValidProperty(validatedProperty)) {
          ...
          console.log(
            `   ✅ Property: ${property.address} - $${property.price.toLocaleString()}`
          )
        } else {
          console.error(
            `   ❌ Invalid property data for ${property.id}, skipping`
          )
        }
      }

      console.log(
        `🎉 Successfully initialized ${SEED_DATA.properties.length} luxury properties across South Florida`
      )
      console.log(
        '📍 Coverage areas: Fisher Island, Bal Harbour, Sunny Isles, Coral Gables, Key Biscayne, Aventura, and more'
      )
      ...
      const priceRanges = {
        ...
        '$2M - $5M': validProperties.filter(
          p => p.price >= 2000000 && p.price < 5000000
        ).length,
        '$5M - $10M': validProperties.filter(
      ...
      console.log(
        `📋 Found ${existingProperties.length} existing properties in database`
      )
    }
  } catch (error) {
...
app.get('/make-server-59c5b421/properties', async c => {
  try {
    ...
    const validProperties = allPropertyItems
      ...
      .filter(property => {
        if (!isValidProperty(property)) {
          console.warn(
            `Invalid property found and filtered out: ${property?.id || 'unknown'}`
          )
          return false
        }
        return true
    ...
    if (query.bedrooms) {
      ...
      if (!isNaN(bedrooms)) {
        properties = properties.filter(
          p => typeof p.bedrooms === 'number' && p.bedrooms >= bedrooms
        )
      }
    }
    ...
    if (query.bathrooms) {
      ...
      if (!isNaN(bathrooms)) {
        properties = properties.filter(
          p => typeof p.bathrooms === 'number' && p.bathrooms >= bathrooms
        )
      }
    }
    ...
    if (query.city) {
      properties = properties.filter(
        p => p.city && p.city.toLowerCase().includes(query.city.toLowerCase())
      )
    }
    ...
    if (query.features) {
      const requestedFeatures = Array.isArray(query.features)
        ? query.features
        : [query.features]
      properties = properties.filter(
        p =>
          Array.isArray(p.features) &&
          requestedFeatures.some(feature =>
            p.features.some(
              pFeature =>
                typeof pFeature === 'string' &&
                typeof feature === 'string' &&
...
app.post('/make-server-59c5b421/leads', async c => {
  try {
    ...
    console.log(
      `🎯 New lead captured: ${lead.firstName} ${lead.lastName} - ${lead.email} (Interest: ${lead.interest})`
    )
...
app.get('/make-server-59c5b421/filters', async c => {
  try {
    ...
    const locations = Array.from(cityMap.entries()).map(([city, count]) => ({
      city,
      count,
    }))
    ...
    const propertyTypes = Array.from(typeMap.entries()).map(
      ([type, count]) => ({
        type,
...
app.post('/make-server-59c5b421/newsletter/subscribe', async c => {
  try {
    ...
    console.log(
      `📧 Newsletter subscription: ${email} (source: ${source || 'website'})`
    )
...
app.get('/make-server-59c5b421/analytics/dashboard', async c => {
  try {
    ...
    const leadMetrics = {
      ...
      thisMonth: leadData.filter(
        l => l.createdAt && new Date(l.createdAt) >= thisMonth
      ).length,
    ...
    const performance = {
      ...
      averagePrice:
        activePrices.length > 0
          ? Math.round(
              activePrices.reduce((sum, price) => sum + price, 0) /
                activePrices.length
            )
          : 0,
      topMarkets: [
